(() => {
  ...
  function bind(){
    // CACHE119：事件必须绑定在“本次新建的弹窗节点”上，不能绑定到永久存在的 modalRoot。
    // 旧版每打开一件装备都会在 modalRoot 累加一个 click 监听器，监听器闭包永久记住旧装备；
    // 切换到第二件装备后，旧监听器先抢到点击并把 busy 设为 true，新监听器因此退出，表现为“只有第一件能洗，重进才恢复”。
    const root=host().querySelector('.forge-backdrop-v210');
    if(!root)return;
    root.querySelector('[data-forge-close]')?.addEventListener('click',close);
    root.addEventListener('click',e=>{if(e.target===root){close();return}});
    ...
    root.addEventListener('click',e=>{
      ...
      if(!item?.id){toast('装备状态已变化，请重新打开装备。','error');return}
