(() => {
  ...
  function latestEventRows(items, limit = 100) {
    const seen = new Set();
    return (Array.isArray(items) ? [...items] : [])
      .sort((a,b) => {
        const ta = new Date(a?.occurred_at || a?.created_at || a?.resolved_at || a?.updated_at || 0).getTime();
        const tb = new Date(b?.occurred_at || b?.created_at || b?.resolved_at || b?.updated_at || 0).getTime();
  ...
  function tabNav() {
    const a = autonomy();
    const eventCount = Math.min((a.recent_events || []).length, 100);
  ...
  function overviewHtml(data) {
    ...
    const events = latestEventRows([...(Array.isArray(data.recent_events) ? data.recent_events : []), ...(Array.isArray(autonomy(data).recent_events) ? autonomy(data).recent_events : [])], 100);
  ...
  function eventsHtml(data) {
    const a=autonomy(data); const items=latestEventRows(a.recent_events || [],100); const settings=a.settings || {}; const states=(Array.isArray(a.states)?a.states:[]).slice(0,100);
    return `<div class="sect-v2-view-bsect02"><div class="sect-v2-callout-bsect01"><strong>分类事件池与人物状态共同驱动宗门</strong><p>系统先按弟子当前行为、性格、心境、关系、职位和宗门政策选择事件家族，再抽取具体模板。当前已安装 ${fmt(a.template_count||0)} 个独立模板、${fmt(a.family_count||0)} 个事件家族；普通事件自动处理，重要事件进入宗主案牍。</p></div>${sectionTitle('自主排期',`${states.length}条 · 最多显示100条`)}<div class="sect-v2-log-bsect01">${states.map(x=>`<article><strong>${esc(x.disciple_name)} · ${esc(activityName(x.activity_name))}</strong><p>${x.next_event_at?countdownSpan(x.next_event_at,'弟子动态 '):'等待排期'} · ${x.next_decision_at?countdownSpan(x.next_decision_at,'行动评估 '):'等待决定'}</p><small>已经历 ${fmt(x.event_count)} 次事件 · ${fmt(x.important_event_count)} 次重要事件</small></article>`).join('')||'<div class="sect-v2-empty-bsect01">暂无自主排期。</div>'}</div>${sectionTitle('最近宗门动态',`最新${items.length}条 · 最多显示100条 · 有变化时显示明细`)}<div class="sect-v2-log-bsect01">${items.map(e=>`<article class="importance-${esc(e.importance_code||'ordinary')}"><strong>${esc(e.title||'宗门动态')}</strong><p>${esc(humanizeEventText(String(e.summary||'').replace(/【结算[：:][^】]+】/g,'').trim()))}</p>${eventOutcomeHtml(e)}<small>${esc(eventCategoryName(e.category_code))} · ${time(e.occurred_at)}</small></article>`).join('')||'<div class="sect-v2-empty-bsect01">尚无新动态。点击结算并刷新会按服务器时间推进宗门。</div>'}</div><div class="sect-v2-warning-bsect02"><strong>容量保护已启用</strong><p>普通明细保留 ${fmt(settings.ordinary_retention_days||90)} 天，并受每宗门 ${fmt(settings.ordinary_hard_cap_per_sect||3000)} 条硬上限约束；重大经历永久保留，旧日常事件按月汇总。</p></div></div>`;
  }
