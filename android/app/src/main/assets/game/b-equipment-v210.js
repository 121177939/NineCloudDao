(() => {
  ...
  const state={overview:null,item:null,busy:false,dirty:false,status:''};
  ...
  function bind(item){
    ...
    root.addEventListener('click',e=>{
      ...
      if(t.matches('[data-forge-reroll]')){const locks=validateLocks(item,false);if(!locks)return;runSocket('reroll_equipment_socket_attributes_v210',{p_item_id:item.id,p_locked_positions:locks,p_request_id:uuid()},`孔位属性已刷新${locks.length?`，已保护${locks.length}孔。`:''}`,locks);return}
  ...
  async function runSocket(name,body,message,keepLocks){
    ...
    }catch(e){const msg=errorText(e);setStatus(msg,'error');toast(msg,'error')}
  ...
  window.B_EQUIPMENT_V210=Object.freeze({module:MODULE,version:'2.1.1-cache115',open,refresh:overview,detailRows});
})();
