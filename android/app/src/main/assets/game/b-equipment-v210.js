(() => {
  ...
  const state={overview:null,item:null,busy:false,dirty:false};
  ...
  function validateLocks(item){const locks=selectedLocks(),m=rerollMeta(item);if(locks.length>3){toast('最多锁定3个孔位。','error');return null}const need=locks.length*m.lockCost;if(need>material('equipment_socket_lock_jade_v210')){toast(`定灵锁玉不足：需要${need}枚。`,'error');return null}return locks}
  function setSocketButtons(item){const root=host(),m=rerollMeta(item),locked=Boolean(item.is_locked),inBag=item.location==='backpack';const attr=root.querySelector('[data-forge-reroll]'),level=root.querySelector('[data-forge-level-all]');if(attr){attr.textContent=`使用${m.short}×${m.rerollCost}`;attr.disabled=state.busy||!inBag||locked}if(level){level.textContent=`使用百炼×${m.levelCost}`;level.disabled=state.busy||!inBag||locked||!m.levelEnabled;level.title=m.levelEnabled?'':'百炼玄铁等级刷新当前已关闭'}}
  ...
  function bind(item){
    ...
    root.addEventListener('click',e=>{
      ...
      if(t.matches('[data-forge-reroll]')){const locks=validateLocks(item);if(!locks)return;runSocket('reroll_equipment_socket_attributes_v210',{p_item_id:item.id,p_locked_positions:locks,p_request_id:uuid()},`孔位属性已刷新${locks.length?`，已保护${locks.length}孔。`:''}`,locks);return}
      if(t.matches('[data-forge-level-all]')){const locks=validateLocks(item);if(!locks)return;runSocket('reroll_equipment_socket_levels_v210',{p_item_id:item.id,p_locked_positions:locks,p_request_id:uuid()},`孔位等级已重炼${locks.length?`，已保护${locks.length}孔。`:''}`,locks);return}
  ...
  async function runSocket(name,body,message,keepLocks){
    if(state.busy)return;state.busy=true;setSocketButtons(state.item);
    try{
      await rpc(name,body);
      state.dirty=true;
      await overview();
      updateDynamic(state.item,keepLocks);
      toast(message||'操作完成。');
  ...
  window.B_EQUIPMENT_V210=Object.freeze({module:MODULE,version:'2.1.1-cache112',open,refresh:overview,detailRows});
})();
