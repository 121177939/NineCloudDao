(() => {
  ...
  function errorText(error) {
    ...
    const map = [
      ['CASINO_GLOBAL_DISABLED', '服务器检查到当前游戏进行非法活动，已暂停此项功能。'],
  ...
  // V1.8.2 CACHE76：保留V1.7.6合并重绘，并接入UI03正式动态牌桌。
  ...
  async function assertCasinoEnabledV198() {
    const data = await rpc('get_casino_feature_switch_v198', {});
    const snapshot = Array.isArray(data) ? data[0] || null : data;
    if (snapshot?.enabled === false || snapshot?.status === 'disabled') throw new Error('CASINO_GLOBAL_DISABLED');
    return snapshot;
  }

  async function loadLobby() {
    await assertCasinoEnabledV198();
