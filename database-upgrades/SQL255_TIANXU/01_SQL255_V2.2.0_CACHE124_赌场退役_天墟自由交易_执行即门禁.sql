-- 九霄问道 V2.2.0 CACHE124 / ADMIN9 R34 / SQL255 R2
...
-- R2：装备托管保护适配生产 move/set_lock RPC 的可选默认参数签名，不再假设固定两参数原函数。
...
begin
  ...
  if not exists(
    ...
    where n.nspname='public' and p.proname='move_equipment_item_bequipment01'
      and p.pronargs>=2 and p.proargtypes[0]='uuid'::regtype::oid and p.proargtypes[1]='text'::regtype::oid
      and (p.pronargs-p.pronargdefaults)<=2
  ) then raise exception 'SQL255_PRECHECK_MOVE_EQUIPMENT_RPC_COMPATIBLE_MISSING'; end if;
  if not exists(
    select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace
    where n.nspname='public' and p.proname='set_equipment_lock_bequipment01'
      and p.pronargs>=2 and p.proargtypes[0]='uuid'::regtype::oid and p.proargtypes[1]='boolean'::regtype::oid
      and (p.pronargs-p.pronargdefaults)<=2
  ) then raise exception 'SQL255_PRECHECK_LOCK_EQUIPMENT_RPC_COMPATIBLE_MISSING'; end if;
end
$precheck$;
...
-- ---------- EQUIPMENT ESCROW GUARD ----------
-- Production may expose move/lock RPCs with extra DEFAULT parameters.
-- Discover the callable overload dynamically, rename it once, and keep the public 2-argument API guarded.
do $wrap_equipment$
declare
  r record;
  v_oid oid;
  ...
  v_result text;
begin
  -- move equipment: if the pre-v255 implementation does not exist yet, locate the production overload
  -- callable with the two client arguments (uuid,text), including overloads with extra DEFAULT args.
  if not exists(
    select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace
    where n.nspname='public' and p.proname='move_equipment_item_bequipment01_pre_v255'
  ) then
    select p.oid,pg_get_function_identity_arguments(p.oid),pg_get_function_result(p.oid)
    ...
    where n.nspname='public' and p.proname='move_equipment_item_bequipment01'
      ...
      and (p.pronargs-p.pronargdefaults)<=2
    order by case when p.pronargs=2 then 0 else 1 end,p.pronargs,p.oid
    limit 1;
    if v_oid is null then raise exception 'SQL255_MOVE_EQUIPMENT_COMPATIBLE_OVERLOAD_NOT_FOUND';end if;
    if v_result<>'jsonb' then raise exception 'SQL255_MOVE_EQUIPMENT_RESULT_UNEXPECTED:%',v_result;end if;
    execute format('alter function public.move_equipment_item_bequipment01(%s) rename to move_equipment_item_bequipment01_pre_v255',v_args);
  end if;

  -- lock equipment: same adaptive handling for optional/default production parameters.
  if not exists(
    select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace
    where n.nspname='public' and p.proname='set_equipment_lock_bequipment01_pre_v255'
  ) then
    select p.oid,pg_get_function_identity_arguments(p.oid),pg_get_function_result(p.oid)
      into v_oid,v_args,v_result
    from pg_proc p join pg_namespace n on n.oid=p.pronamespace
    where n.nspname='public' and p.proname='set_equipment_lock_bequipment01'
      and p.pronargs>=2
      and p.proargtypes[0]='uuid'::regtype::oid
      and p.proargtypes[1]='boolean'::regtype::oid
      and (p.pronargs-p.pronargdefaults)<=2
    order by case when p.pronargs=2 then 0 else 1 end,p.pronargs,p.oid
    limit 1;
    if v_oid is null then raise exception 'SQL255_LOCK_EQUIPMENT_COMPATIBLE_OVERLOAD_NOT_FOUND';end if;
    if v_result<>'jsonb' then raise exception 'SQL255_LOCK_EQUIPMENT_RESULT_UNEXPECTED:%',v_result;end if;
    execute format('alter function public.set_equipment_lock_bequipment01(%s) rename to set_equipment_lock_bequipment01_pre_v255',v_args);
  end if;

  -- Renamed originals are implementation details. Revoke every overload so clients cannot bypass escrow guards.
  for r in
    select p.proname,pg_get_function_identity_arguments(p.oid) identity_args
    from pg_proc p join pg_namespace n on n.oid=p.pronamespace
    where n.nspname='public' and p.proname in('move_equipment_item_bequipment01_pre_v255','set_equipment_lock_bequipment01_pre_v255')
  loop
...
create or replace function public.move_equipment_item_bequipment01(p_item_id uuid,p_target_location text)
returns jsonb language plpgsql security definer set search_path='' as $$
declare v_result jsonb;
begin
  if exists(select 1 from public.character_equipment_items_bequipment01 where id=p_item_id and tianxu_escrowed_v255) then raise exception 'TIANXU_EQUIPMENT_ESCROWED';end if;
  execute 'select public.move_equipment_item_bequipment01_pre_v255($1,$2)' into v_result using p_item_id,p_target_location;
  return v_result;
end $$;
create or replace function public.set_equipment_lock_bequipment01(p_item_id uuid,p_locked boolean)
returns jsonb language plpgsql security definer set search_path='' as $$
declare v_result jsonb;
begin
  if exists(select 1 from public.character_equipment_items_bequipment01 where id=p_item_id and tianxu_escrowed_v255) then raise exception 'TIANXU_EQUIPMENT_ESCROWED';end if;
  execute 'select public.set_equipment_lock_bequipment01_pre_v255($1,$2)' into v_result using p_item_id,p_locked;
  return v_result;
end $$;

revoke all on function public.move_equipment_item_bequipment01(uuid,text),public.set_equipment_lock_bequipment01(uuid,boolean) from public,anon;
grant execute on function public.move_equipment_item_bequipment01(uuid,text),public.set_equipment_lock_bequipment01(uuid,boolean) to authenticated;
...
begin
  ...
  -- Disable casino cron work.
  if to_regclass('cron.job') is not null then
    update cron.job set active=false where active and (command ilike '%casino%' or command ilike '%paigow%' or command ilike '%fish_shrimp%' or command ilike '%spirit_dice%');
  end if;
...
returns jsonb language plpgsql security definer set search_path='' as $$
declare v_rel text;v_casino_exec bigint;v_cron bigint:=0;
begin
  ...
  if to_regclass('cron.job') is not null then select count(*) into v_cron from cron.job where active and (command ilike '%casino%' or command ilike '%paigow%' or command ilike '%fish_shrimp%' or command ilike '%spirit_dice%');end if;
  return jsonb_build_object('status',case when v_rel is not null and v_casino_exec=0 and v_cron=0 then 'PASS' else 'FAIL' end,'technique_book_relation',v_rel,'retired_player_executable_functions',v_casino_exec,'active_retired_game_cron',v_cron,'settings',(select to_jsonb(s) from public.tianxu_settings_v255 s where singleton_id=1));
end $$;
...
-- ---------- GATE ----------
do $gate$
declare v_check jsonb;v_set public.tianxu_settings_v255%rowtype;v_rel text;v_casino_exec bigint;v_cron bigint:=0;
begin
  ...
  if to_regprocedure('public.move_equipment_item_bequipment01(uuid,text)') is null or to_regprocedure('public.set_equipment_lock_bequipment01(uuid,boolean)') is null then raise exception 'SQL255_GATE_EQUIPMENT_ESCROW_GUARD_MISSING';end if;
  if not exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='move_equipment_item_bequipment01_pre_v255')
     or not exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='set_equipment_lock_bequipment01_pre_v255') then raise exception 'SQL255_GATE_EQUIPMENT_ORIGINAL_NOT_ARCHIVED';end if;
  if exists(
    select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace
    where n.nspname='public' and p.proname in('move_equipment_item_bequipment01_pre_v255','set_equipment_lock_bequipment01_pre_v255')
      and (has_function_privilege('authenticated',p.oid,'EXECUTE') or has_function_privilege('anon',p.oid,'EXECUTE'))
  ) then raise exception 'SQL255_GATE_EQUIPMENT_ESCROW_BYPASS_EXECUTABLE';end if;
  if position('tianxu_escrowed_v255' in pg_get_functiondef('public.move_equipment_item_bequipment01(uuid,text)'::regprocedure))=0
     or position('tianxu_escrowed_v255' in pg_get_functiondef('public.set_equipment_lock_bequipment01(uuid,boolean)'::regprocedure))=0 then raise exception 'SQL255_GATE_EQUIPMENT_ESCROW_GUARD_BODY_MISSING';end if;
  ...
  if to_regclass('cron.job') is not null then select count(*) into v_cron from cron.job where active and (command ilike '%casino%' or command ilike '%paigow%' or command ilike '%fish_shrimp%' or command ilike '%spirit_dice%');if v_cron<>0 then raise exception 'SQL255_GATE_CASINO_CRON_STILL_ACTIVE:%',v_cron;end if;end if;
end
$gate$;

commit;
select 'SQL255_GATE_PASSED'::text as result,
       'V2.2.0 CACHE124 / ADMIN9 R34 / 天墟 ONLINE；SQL255 R2自适应装备RPC托管保护；赌场玩家入口与RPC权限已退役；NEXT SQL256'::text as detail;
