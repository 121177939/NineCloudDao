-- 九霄问道 SQL264 R3 · V2.2.0 CACHE136 / ADMIN9 R40
...
-- R3修复：生产库 character_cultivation_effects.source_type 的 CHECK 约束未包含 tiandao_companion。
--         本版在同一事务内“保留原 CHECK 表达式 + 仅追加 tiandao_companion”，再写入道侣标记行；不复用/冒充其他来源类型。
  ...
  if v_dynamic_rate<=0 then
    ...
    if v_delta>0 then
      ...
      from public.grant_cultivation_capped_v1(
        ...
        jsonb_build_object('sql','SQL264 R3','heaven_mode',coalesce(v_mode,'auto'),'companion_multiplier',v_companion_multiplier)
      ) g;
...
comment on function public.claim_cultivation_v1() is 'SQL264 R3 CACHE136: wraps SQL263 authoritative cultivation settlement; fixed heaven mode is applied to real gains and active NPC companion multiplies final rate exactly x1.5.';


-- ---------- extend cultivation effect source type CHECK safely ----------
-- SQL263 production constraint rejects source_type='tiandao_companion'.
-- Preserve the complete original CHECK expression and append exactly one legal source type.
do $extend_effect_source_type$
declare
  v_oid oid;
  v_expr text;
  v_validated boolean;
begin
  select c.oid,pg_get_expr(c.conbin,c.conrelid),c.convalidated
    into v_oid,v_expr,v_validated
  ...
  if v_oid is null then
    raise exception 'SQL264_PRECHECK_CULTIVATION_EFFECTS_SOURCE_TYPE_CONSTRAINT_MISSING';
  end if;
  if v_expr is null or position('source_type' in lower(v_expr))=0 then
    raise exception 'SQL264_PRECHECK_CULTIVATION_EFFECTS_SOURCE_TYPE_CONSTRAINT_UNEXPECTED:%',coalesce(v_expr,'NULL');
  end if;

  -- Idempotent: if a previous successful SQL264 already added the type, keep the existing constraint untouched.
  if position('tiandao_companion' in v_expr)>0 then return; end if;

  execute 'alter table public.character_cultivation_effects drop constraint character_cultivation_effects_source_type_check';
  execute format(
    'alter table public.character_cultivation_effects add constraint character_cultivation_effects_source_type_check check ((%s) or source_type = %L) not valid',
    v_expr,'tiandao_companion'
  );

  -- Existing rows already satisfied the original validated CHECK; the new OR branch only widens it.
  if coalesce(v_validated,true) then
    execute 'alter table public.character_cultivation_effects validate constraint character_cultivation_effects_source_type_check';
  end if;
end
$extend_effect_source_type$;
...
begin
  ...
  select pg_get_expr(c.conbin,c.conrelid) into v_source_type_constraint
  from pg_constraint c
  where c.conrelid='public.character_cultivation_effects'::regclass
    and c.contype='c'
    and c.conname='character_cultivation_effects_source_type_check'
  limit 1;
  if v_source_type_constraint is null or position('tiandao_companion' in v_source_type_constraint)=0 then
    raise exception 'SQL264_GATE_COMPANION_SOURCE_TYPE_NOT_ALLOWED';
  end if;
  ...
  select count(*) into v_bad_effects from public.character_cultivation_effects
    where source_key='tiandao_companion_v264' and source_type<>'tiandao_companion';
  if v_bad_effects<>0 then raise exception 'SQL264_GATE_COMPANION_SOURCE_TYPE_WRONG:%',v_bad_effects; end if;
end
$gate$;
...
select jsonb_build_object(
  'sql','264 R3','gate','SQL264_GATE_PASSED','release','V2.2.0 CACHE136',
