-- 九霄问道 SQL259 · V2.2.0 CACHE128
-- 天道人物 / 缘遇 / 人物志 / 仙缘 / 道侣
-- 彻底替换旧红尘玩家入口；旧表仅保留历史归档，不再作为新版运行依赖。
-- 生产基线：SQL258 ONLINE；成功后 NEXT SQL260。
...
create table if not exists public.tiandao_people_settings_v259(
  ...
  ai_engine text not null default 'server_personality_v1',
...
returns jsonb language plpgsql security definer set search_path='' as $$
declare v_settings jsonb;v_npcs jsonb;v_rel jsonb;v_enc jsonb;v_comp jsonb;v_audit jsonb;
begin
  ...
  return jsonb_build_object('status','ok','settings',v_settings,'counts',jsonb_build_object('npc_total',(select count(*) from public.tiandao_npcs_v259),'core_ai',(select count(*) from public.tiandao_npcs_v259 where core_ai),'romanceable',(select count(*) from public.tiandao_npcs_v259 where romanceable),'relations',(select count(*) from public.tiandao_relations_v259),'pending_encounters',(select count(*) from public.tiandao_encounters_v259 where status='pending' and expires_at>clock_timestamp()),'active_companions',(select count(*) from public.tiandao_companions_v259 where status='active')),'npcs',v_npcs,'recent_relations',v_rel,'recent_encounters',v_enc,'companions',v_comp,'audit',v_audit);
end $$;
...
begin
  ...
  update public.tiandao_people_settings_v259 set
    ...
    confess_affinity_min=coalesce((p_patch->>'confess_affinity_min')::integer,confess_affinity_min),confess_trust_min=coalesce((p_patch->>'confess_trust_min')::integer,confess_trust_min),confess_intimacy_min=coalesce((p_patch->>'confess_intimacy_min')::integer,confess_intimacy_min),confess_romance_min=coalesce((p_patch->>'confess_romance_min')::integer,confess_romance_min),updated_at=clock_timestamp()
...
returns jsonb language plpgsql security definer set search_path='' as $$
declare v_old_exposed integer;v_table_exposed integer;
begin
  perform public.admin_whoami_v1();
  ...
  select count(*) into v_table_exposed from information_schema.table_privileges where table_schema='public' and table_name like 'tiandao_%_v259' and grantee in('anon','authenticated','PUBLIC');
  return jsonb_build_object('status',case when (select count(*) from public.tiandao_npcs_v259)=50 and (select count(*) from public.tiandao_npcs_v259 where core_ai)=20 and (select count(*) from public.tiandao_npcs_v259 where romanceable)=30 and v_old_exposed=0 and v_table_exposed=0 then 'PASS' else 'FAIL' end,
    'npc_total',(select count(*) from public.tiandao_npcs_v259),'core_ai',(select count(*) from public.tiandao_npcs_v259 where core_ai),'romanceable',(select count(*) from public.tiandao_npcs_v259 where romanceable),'romance_female',(select count(*) from public.tiandao_npcs_v259 where romanceable and gender='female'),'romance_male',(select count(*) from public.tiandao_npcs_v259 where romanceable and gender='male'),'underage',(select count(*) from public.tiandao_npcs_v259 where age<18),'legacy_player_rpc_exposed',v_old_exposed,'raw_table_privileges',v_table_exposed,
    'player_rpc_count',(select count(*) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname in('get_tiandao_people_hub_v1','get_tiandao_person_detail_v1','resolve_tiandao_encounter_v1','tiandao_npc_interact_v1','tiandao_romance_action_v1','tiandao_companion_action_v1')));
end $$;
...
grant execute on function public.resolve_tiandao_encounter_v1(uuid,text) to authenticated;
grant execute on function public.tiandao_npc_interact_v1(uuid,text) to authenticated;
grant execute on function public.tiandao_romance_action_v1(uuid,text,text) to authenticated;
grant execute on function public.tiandao_companion_action_v1(text) to authenticated;
...
-- ---------- FINAL GATE ----------
do $gate$
declare v_old_exposed integer;v_table_exposed integer;v_count integer;
begin
  ...
  select count(*) into v_count from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname in('get_tiandao_people_hub_v1','get_tiandao_person_detail_v1','resolve_tiandao_encounter_v1','tiandao_npc_interact_v1','tiandao_romance_action_v1','tiandao_companion_action_v1');
  if v_count<>6 then raise exception 'SQL259_GATE_PLAYER_RPC_COUNT:%',v_count;end if;
  select count(*) into v_old_exposed from information_schema.routine_privileges where specific_schema='public' and routine_name in('get_npc_social_v1','interact_with_npc_v1','form_npc_relationship_v1') and grantee in('anon','authenticated','PUBLIC') and privilege_type='EXECUTE';
  if v_old_exposed<>0 then raise exception 'SQL259_GATE_LEGACY_SOCIAL_RPC_STILL_EXPOSED:%',v_old_exposed;end if;
  ...
  if v_table_exposed<>0 then raise exception 'SQL259_GATE_RAW_TABLE_PRIVILEGE_EXPOSED:%',v_table_exposed;end if;
  if not has_function_privilege('authenticated','public.get_tiandao_people_hub_v1()','EXECUTE') then raise exception 'SQL259_GATE_HUB_EXECUTE_MISSING';end if;
  if has_function_privilege('anon','public.get_tiandao_people_hub_v1()','EXECUTE') then raise exception 'SQL259_GATE_ANON_HUB_EXPOSED';end if;
