# 九霄问道 V2.1.1 CACHE120

当前客户端基线：**V2.1.1 CACHE120**；Android：**versionCode 2001420 / versionName 2.1.1-cache120**；GM：**ADMIN9 R27**。
...
- CACHE120 改为把监听器绑定到每次新建的 `.forge-backdrop-v210`；关闭弹窗时DOM与监听器一起销毁。
- 所有孔位洗炼、百炼、升品、破境、器魂承接按钮在点击时实时读取 `state.item`，不再使用弹窗打开时捕获的旧装备。
- CACHE118 GitHub Pages构建修复和CACHE117/116装备数据库规则全部保留。

数据库仍为 **SQL247 R2 已上线**；CACHE120无新增SQL，下一编号仍为 **SQL248**。
