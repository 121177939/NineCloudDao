...
ck('version',text('VERSION.txt').strip()=='V1.4')
ck('config',all(x in text('config.js') for x in ["buildId: 'v1-4-cache42'",'cacheEpoch: 42']))
ck('index','b-paigow01.js?v=v1-4-cache42' in text('index.html'))
ck('iframe','paigow-app.js?v=v1-4-cache42' in text('b-paigow01.html'))
ck('sw','nine-cloud-dao-v1.4-cache42' in text('sw.js'))
ck('feature-assets','pg-resource-fx-layer' in text('paigow-app.css') and 'delete_paigow_room_bpaigow01' in text('paigow-app.js'))
manifest=json.loads(text('PAGES_ARTIFACT_MANIFEST.json'));ck('manifest-meta',manifest.get('version')=='V1.4 CACHE42' and manifest.get('clientBuild')=='v1-4-cache42')
