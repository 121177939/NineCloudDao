...
checks={
 'version':(root/'VERSION.txt').read_text('utf-8').strip()=='V1.7.2',
 'config':all(x in (root/'config.js').read_text('utf-8') for x in ["version: '1.7.2'","releaseLabel: 'V1.7.2 CACHE48'","buildId: 'v1-7-2-cache48'",'cacheEpoch: 48']),
 'index':all(x in (root/'index.html').read_text('utf-8') for x in ['b-paigow01.js?v=v1-7-2-cache48','b-equipment01.js?v=v1-7-2-cache48','b-equipment01.css?v=v1-7-2-cache48']),
 'iframe':all(x in (root/'b-paigow01.html').read_text('utf-8') for x in ['paigow-realtime.js?v=v1-7-2-cache48','paigow-app.js?v=v1-7-2-cache48']),
 'service-worker':all(x in (root/'sw.js').read_text('utf-8') for x in ['nine-cloud-dao-v1.7.2-cache48','paigow-realtime.js?v=v1-7-2-cache48','b-equipment01.js?v=v1-7-2-cache48']),
 ...
 'equipment-single-cave-grid':all(x in (root/'b-equipment01.js').read_text('utf-8') for x in ["cave.hidden=false",'renderCaveEquipmentIntoNative','storagePanel(\'backpack\')']) and 'data-eq-view="cave"' not in (root/'b-equipment01.js').read_text('utf-8'),
}
...
(out/'PAGES_ARTIFACT_MANIFEST.json').write_text(json.dumps({'version':'V1.7.2 CACHE48','clientBuild':'v1-7-2-cache48','files':manifest},ensure_ascii=False,indent=2)+'\n','utf-8')
...
print(f'V1.7.2 CACHE48 pages build PASS files={len(manifest)}')
