...
checks={
 'version':text('VERSION.txt').strip()=='V1.7.6',
 'config':all(x in text('config.js') for x in ["version: '1.7.6'","releaseLabel: 'V1.7.6 CACHE55'","buildId: 'v1-7-6-cache55'",'cacheEpoch: 55']),
 'index':all(x in text('index.html') for x in ['<!-- version: V1.7.6 CACHE55 -->','b-paigow01.js?v=v1-7-6-cache55','b-equipment01.js?v=v1-7-6-cache55','b-equipment01.css?v=v1-7-6-cache55']),
 'iframe-b02':all(x in text('b-paigow01.html') for x in ['paigow-realtime.js?v=v1-7-6-cache55','paigow-app.js?v=v1-7-6-cache55','b-paigow02-ui.css?v=v1-7-6-cache55','b-paigow02-ui.js?v=v1-7-6-cache55']),
 'paigow-b02-no-network-hooks':all(x not in b02 for x in ['fetch(','XMLHttpRequest','WebSocket','setInterval(','MutationObserver']),
 'service-worker':all(x in sw for x in ['nine-cloud-dao-v1.7.6-cache55','paigow-realtime.js?v=v1-7-6-cache55','b-equipment01.js?v=v1-7-6-cache55','b-paigow02-ui.css?v=v1-7-6-cache55','b-paigow02-ui.js?v=v1-7-6-cache55']),
 'workflow-builder-present':"python3 tools/build_pages_v1_7_6.py" in wf and (root/'tools/build_pages_v1_7_6.py').is_file(),
...
(out/'PAGES_ARTIFACT_MANIFEST.json').write_text(json.dumps({'version':'V1.7.6 CACHE55','clientBuild':'v1-7-6-cache55','files':manifest},ensure_ascii=False,indent=2)+'\n','utf-8')
...
print(f'V1.7.6 CACHE55 pages build PASS files={len(manifest)}')
