...
checks={
 'version':(root/'VERSION.txt').read_text('utf-8').strip()=='V1.7.5.1',
 'config':all(x in (root/'config.js').read_text('utf-8') for x in ["version: '1.7.5.1'","releaseLabel: 'V1.7.5.1 CACHE52'","buildId: 'v1-7-5-1-cache52'",'cacheEpoch: 52']),
 'index':all(x in (root/'index.html').read_text('utf-8') for x in ['b-paigow01.js?v=v1-7-5-1-cache52','b-equipment01.js?v=v1-7-5-1-cache52','b-equipment01.css?v=v1-7-5-1-cache52']),
 'iframe':all(x in (root/'b-paigow01.html').read_text('utf-8') for x in ['paigow-realtime.js?v=v1-7-5-1-cache52','paigow-app.js?v=v1-7-5-1-cache52']),
 'service-worker':all(x in (root/'sw.js').read_text('utf-8') for x in ['nine-cloud-dao-v1.7.5.1-cache52','paigow-realtime.js?v=v1-7-5-1-cache52','b-equipment01.js?v=v1-7-5-1-cache52']),
...
checks.update({
 ...
 'casino-dice-win-glow-cache52':all(x in (root/'app.js').read_text('utf-8') for x in ['spiritDiceChoiceHitV175(results, choiceCode)',"spiritDiceChoiceHitV175(results,code)","hit?'win':''"]),
})
...
(out/'PAGES_ARTIFACT_MANIFEST.json').write_text(json.dumps({'version':'V1.7.5.1 CACHE52','clientBuild':'v1-7-5-1-cache52','files':manifest},ensure_ascii=False,indent=2)+'\n','utf-8')
...
print(f'V1.7.5.1 CACHE52 pages build PASS files={len(manifest)}')
