...
checks={
 ...
 'config':all(x in text('config.js') for x in [label,build,'cacheEpoch: 107']),
 'release-json':release.get('releaseLabel')==label and release.get('cacheEpoch')==107 and release.get('buildId')==build and release.get('nextSqlNumber')==236,
 'database':baseline.get('nextSqlNumber')==236 and baseline.get('sqlRevision')=='211-221 + 229-235',
