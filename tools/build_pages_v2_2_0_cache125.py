...
build='v2-2-0-cache125-tianxu-admin34-sql255'; label='V2.2.0 CACHE125'
required=[
...
'pages-lock':'PAGES_DEPLOY default-github-pages-artifact-r4' in text('VERSION.txt'),
'config':all(x in text('config.js') for x in [label,build,'cacheEpoch: 125']),
'release-json':release.get('releaseLabel')==label and release.get('cacheEpoch')==125 and release.get('buildId')==build and release.get('nextSqlNumber')==256 and release.get('gmVersion')=='ADMIN9 R34' and release.get('runtimeDatabaseGate')=='SQL255_GATE_PASSED',
...
'index-tianxu':all(x in idx for x in ['b-tianxu-v220.css','<!-- version: V2.2.0 CACHE125 -->']) and not any(x in idx.lower() for x in ['paigow','赌场','赌坊']),
