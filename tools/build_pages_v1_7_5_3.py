...
checks={
 'version':(root/'VERSION.txt').read_text('utf-8').strip()=='V1.7.5.3',
 'config':all(x in (root/'config.js').read_text('utf-8') for x in ["version: '1.7.5.3'","releaseLabel: 'V1.7.5.3 CACHE54'","buildId: 'v1-7-5-3-cache54'",'cacheEpoch: 54']),
 'index':all(x in (root/'index.html').read_text('utf-8') for x in ['b-paigow01.js?v=v1-7-5-3-cache54','b-equipment01.js?v=v1-7-5-3-cache54','b-equipment01.css?v=v1-7-5-3-cache54']),
 'iframe':all(x in (root/'b-paigow01.html').read_text('utf-8') for x in ['paigow-realtime.js?v=v1-7-5-3-cache54','paigow-app.js?v=v1-7-5-3-cache54']),
 'service-worker':all(x in (root/'sw.js').read_text('utf-8') for x in ['nine-cloud-dao-v1.7.5.3-cache54','paigow-realtime.js?v=v1-7-5-3-cache54','b-equipment01.js?v=v1-7-5-3-cache54']),
...
checks.update({
 ...
 'casino-dice-win-glow-cache54':all(x in (root/'app.js').read_text('utf-8') for x in [
   'spiritDiceHasAcceptedBetV175','spiritDiceRememberAcceptedBetsV175',"button.classList.toggle('dice-win-glow',shouldGlow)"
 ]) and all(x in (root/'styles.css').read_text('utf-8') for x in ['.dice-target-card.dice-win-glow','.dice-target-card.dice-win-glow:disabled']),
 'workflow-builder-present':"python3 tools/build_pages_v1_7_5_3.py" in (root/'.github/workflows/deploy-pages.yml').read_text('utf-8') and (root/'tools/build_pages_v1_7_5_3.py').is_file(),
})
...
(out/'PAGES_ARTIFACT_MANIFEST.json').write_text(json.dumps({'version':'V1.7.5.3 CACHE54','clientBuild':'v1-7-5-3-cache54','files':manifest},ensure_ascii=False,indent=2)+'\n','utf-8')
...
print(f'V1.7.5.3 CACHE54 pages build PASS files={len(manifest)}')
