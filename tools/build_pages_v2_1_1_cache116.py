...
build='v2-1-1-cache116-socketrerollfix-admin26r2-sql246'
label='V2.1.1 CACHE116'
required=[
...
checks={
 ...
 'config':all(x in text('config.js') for x in [label,build,'cacheEpoch: 116']),
 'release-json':release.get('releaseLabel')==label and release.get('cacheEpoch')==116 and release.get('buildId')==build and release.get('nextSqlNumber')==247,
 'database':baseline.get('nextSqlNumber')==247 and 'SQL246 REQUIRED' in baseline.get('sqlRevision',''),
 ...
 'cache116-release':release.get('gmVersion')=='ADMIN9 R26 R2' and release.get('migrationSql')==[245,246] and 'SQL246_GATE_PASSED' in release.get('runtimeDatabaseGate',''),
 'cache116-equipped-detail':"if (inBag) actions.push('<button class=\"primary-btn\" data-eq-action=\"forge\">孔位 / 升品 / 破境</button>');" in text('b-equipment01.js') and "inBag || equipped" not in text('b-equipment01.js'),
