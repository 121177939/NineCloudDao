...
build_id = 'v1-8-4-cache83-bsect02-deployfix1'
label = 'V1.8.4 CACHE83'
required = [
...
checks = {
    ...
    'config': all(t in text('config.js') for t in [f"releaseLabel: '{label}'", f"buildId: '{build_id}'", 'cacheEpoch: 83']),
    'baseline': baseline.get('sourceBaseline') == 'V1.8.3 CACHE82' and baseline.get('clientHotfix') == 'BSECT02_DEPLOYFIX1' and baseline.get('gmVersion') == 'ADMIN9 R13',
    'database': baseline.get('phase3SqlStart') == 191 and '185-190' in str(baseline.get('sqlRevision')),
    ...
    'cache': 'nine-cloud-dao-v1.8.4-cache83-bsect02-deployfix1' in sw and build_id in sw,
    'no-secrets': 'sb_secret_' not in '\n'.join(text(x) for x in ['app.js','config.js','index.html']).lower(),
    'sect': all((root / x).is_file() for x in ['b-sect-v2.js','b-sect-v2.css']) and 'B-SECT02' in text('b-sect-v2.js') and 'sectV2RootBSect01' in app and 'sect-focus-mode' in app,
