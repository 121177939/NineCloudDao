...
run('release metadata',[py,'tools/verify_release_v1_2_fix2.py','.'])
for rel in ['app.js','config.js','update-guard.js','sw.js','b-paigow01.js','paigow-app.js']:
 run('node syntax '+rel,['node','--check',rel])
for rel in ['tools/prepare_pages_site_v1_2_fix2.py','tools/verify_pages_site_v1_2_fix2.py','tools/verify_release_v1_2_fix2.py','tools/sql_static_audit_v1_2_fix2.py','tools/validate_paigow_v26_ui.py','tools/ci_v1_2_fix2.py']:
 run('python syntax '+rel,[py,'-m','py_compile',rel])
run('V1.2 FIX2 SQL audit',[py,'tools/sql_static_audit_v1_2_fix2.py','.'])
run('V26 UI static audit',[py,'tools/validate_paigow_v26_ui.py','.'])
run('V1.2 FIX2 feature test',['node','tools/test_features_v1_2_fix2.js','.'])
...
run('build pages',[py,'tools/prepare_pages_site_v1_2_fix2.py','.','.pages-site'])
run('verify pages',[py,'tools/verify_pages_site_v1_2_fix2.py','.pages-site'])
print('\nV1.2 FIX2 CACHE39 CI PASS')
