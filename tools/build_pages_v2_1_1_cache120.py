...
build='v2-1-1-cache120-forgefastpath-equipmentstate-admin27-sql247'
label='V2.1.1 CACHE120'
required=[
...
checks={
 ...
 'config':all(x in text('config.js') for x in [label,build,'cacheEpoch: 120']),
 'release-json':release.get('releaseLabel')==label and release.get('cacheEpoch')==120 and release.get('buildId')==build and release.get('nextSqlNumber')==248,
 ...
 'cache120-release':release.get('gmVersion')=='ADMIN9 R27' and release.get('migrationSql')==[245,246,247] and release.get('runtimeDatabaseGate')=='SQL247_GATE_PASSED_PRODUCTION_NO_NEW_SQL_FOR_CACHE120',
 'cache120-equipped-detail':"if (inBag) actions.push('<button class=\"primary-btn\" data-eq-action=\"forge\">孔位 / 升品 / 破境</button>');" in text('b-equipment01.js') and "inBag || equipped" not in text('b-equipment01.js'),
 ...
 'cache120-forge-listener-lifecycle':"const root=host().querySelector('.forge-backdrop-v210');" in text('b-equipment-v210.js') and "const item=state.item;" in text('b-equipment-v210.js') and "function bind(item)" not in text('b-equipment-v210.js') and "const root=host();" not in text('b-equipment-v210.js').split('function bind(){',1)[1].split('async function runSocket',1)[0],
 'pages-workflow-cache120':all(x in text('.github/workflows/deploy-pages.yml') for x in ['V2.1.1 CACHE120','build_pages_v2_1_1_cache120.py']) and 'build_pages_v2_1_1_cache116.py' not in text('.github/workflows/deploy-pages.yml'),
 'cache120-forge-fast-path':all(x in text('b-equipment-v210.js') for x in ['applySocketResult','client_elapsed_ms','正在洗炼…','SQL246 RPC自身在同一事务内强校验']) and 'await requireBackpackCurrent(state.item)' not in text('b-equipment-v210.js').split('async function runSocket',1)[1].split('async function run(',1)[0] and 'await overview()' not in text('b-equipment-v210.js').split('async function runSocket',1)[1].split('async function run(',1)[0] and 'await freshItem(state.item)' not in text('b-equipment-v210.js').split('async function runSocket',1)[1].split('async function run(',1)[0],
 'cache120-result-patch':all(x in text('b-equipment-v210.js') for x in ['result.sockets','result.material_after','result.lock_item_after','result.spirit_stones_after']),
