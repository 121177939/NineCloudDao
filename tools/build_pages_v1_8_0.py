...
build_id='v1-8-2-cache73-equipmentfx1-uifix1'
label='V1.8.2 CACHE73'
...
checks={
 'version':text('VERSION.txt').splitlines()[0].strip()=='V1.8.2 CACHE73' and build_id in text('VERSION.txt'),
 'config':all(t in text('config.js') for t in ["version: '1.8.2'","releaseLabel: 'V1.8.2 CACHE73'",f"buildId: '{build_id}'",'cacheEpoch: 73']),
 ...
 'baseline-lock':baseline.get('developmentBaseline')=='V1.8.2_AB57_ADMIN9_CACHE73' and baseline.get('gmVersion')=='ADMIN9 R11 FIX1',
 'index-assets':all(t in text('index.html') for t in ['<!-- version: V1.8.2 CACHE73 -->',f'b-secret-realm01.js?v={build_id}',f'b-equipment01.js?v={build_id}',f'b-paigow01.js?v={build_id}']),
 'service-worker':all(t in sw for t in ['nine-cloud-dao-v1.8.2-cache73-equipmentfx1-uifix1',f'b-equipment01.js?v={build_id}',f'paigow-app.js?v={build_id}']) and all(t not in sw for t in ['gm-admin','gm-operations','ADMIN9']),
 ...
 'equipment-fx1':all(t in equipment for t in ['function equipmentEffectTier(item)','equipment-effect-tier6-bequipment01','equipment-effect-tier8-bequipment01','equipment-effect-tier10-bequipment01']) and all(t in text('b-equipment01.css') for t in ['CACHE73 EQUIPMENT-FX1-UIFIX1','equipmentFxInnerSpinBEquipment01','equipmentFxOuterSpinBEquipment01','#c99a32 0%','#4f86d9 20%','#925fd1 40%','#e05252 60%','#f2d06b 80%']),
