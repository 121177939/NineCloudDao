...
build_id='v1-8-3-cache77-mutationatk1'
label='V1.8.3 CACHE77'
...
checks={
 'version':text('VERSION.txt').splitlines()[0].strip()=='V1.8.3 CACHE77' and build_id in text('VERSION.txt'),
 'config':all(t in text('config.js') for t in ["version: '1.8.3'","releaseLabel: 'V1.8.3 CACHE77'",f"buildId: '{build_id}'",'cacheEpoch: 77']),
 'deployment-enabled':baseline.get('deploymentStatus')=='formal_release' and release.get('deploymentAllowed') is True and baseline.get('sqlRevision')=='143-166',
 'local-gm-mode':baseline.get('gmDeliveryMode')=='local_only' and release.get('gmDeliveryMode')=='local_only',
 'baseline-lock':baseline.get('developmentBaseline')=='V1.8.3_AB61_ADMIN9_CACHE77' and baseline.get('gmVersion')=='ADMIN9 R11 FIX4',
 'index-assets':all(t in text('index.html') for t in ['<!-- version: V1.8.3 CACHE77 -->',f'b-secret-realm01.js?v={build_id}',f'b-equipment01.js?v={build_id}',f'b-paigow01.js?v={build_id}']),
 'service-worker':all(t in sw for t in ['nine-cloud-dao-v1.8.3-cache77-mutationatk1',f'b-equipment01.js?v={build_id}',f'paigow-app.js?v={build_id}']) and all(t not in sw for t in ['gm-admin','gm-operations','ADMIN9']),
 ...
 'mutation-attack1':all(t in app for t in ['mutation_base_stat_multiplier','realm_base_attack','境界提供的基础道攻提高','变异灵根不再放大戒指效果']) and all(t in json.dumps(baseline,ensure_ascii=False) for t in ['只使境界基础道攻提高8%','不放大装备、强化、功法或戒指数值']),
 'sword-ring-preserved':all(t in app for t in ['sword_heart_active','剑心使戒指效果提高','effective_equipment_element_bonus','talent_ring_amplification_rate']),
 'equipment-fx1':all(t in equipment for t in ['function equipmentEffectTier(item)','equipment-effect-tier6-bequipment01','equipment-effect-tier8-bequipment01','equipment-effect-tier10-bequipment01']) and all(t in text('b-equipment01.css') for t in ['CACHE75 EQUIPMENT-FX1-UIFIX3','equipmentFxInnerSpinBEquipment01','equipmentFxOuterSpinBEquipment01','#c99a32 0%','#4f86d9 20%','#925fd1 40%','#e05252 60%','#f2d06b 80%']),
 'equipped-card-uifix2':'CACHE75 EQUIPMENT-FX1-UIFIX3' in text('b-equipment01.css') and '<small>${esc(item.realm_name)}</small><small>${esc(item.main_stat_display)}</small>' not in equipment and '<small>${esc(item.main_stat_display)}</small><small>${esc(item.socket_display)}</small>' in equipment and 'translateY(4px)' in text('b-equipment01.css'),
 ...
 'equipment-grid-grade-corner':all(t in equipment for t in ['function gridGradeCornerLabel(item)','equipment-grade-corner-bequipment01','<em>${esc(gridSocketDisplay(item))}</em>']) and '<span class="equipment-grade-bequipment01">${esc(item.grade_name)}</span>' not in equipment and all(t in text('b-equipment01.css') for t in ['CACHE75 EQUIPMENT-FX1-UIFIX3','equipment-grade-corner-bequipment01','right:3px;top:3px']),
 ...
 'no-old-build':build_id in blob and not any(x in blob for x in ['v1-7-7-fix1-cache57','v1-7-8-cache58-equipfix2','v1-7-9-cache59-secretclaim2-history5','v1-8-1-cache65-secret-realm-reward2','v1-8-2-cache76-equipmentfx1-uifix4']),
