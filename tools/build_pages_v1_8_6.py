...
build_id = 'v1-8-6-cache88-bsect04-socialspar1'
label = 'V1.8.6 CACHE88'
required = [
...
checks = {
    ...
    'config': all(t in text('config.js') for t in [f"releaseLabel: '{label}'", f"buildId: '{build_id}'", 'cacheEpoch: 88']),
    'baseline': baseline.get('sourceBaseline') == 'V1.8.5 CACHE86' and baseline.get('clientHotfix') == 'BSECT04_SOCIALSPAR1' and baseline.get('gmVersion') == 'ADMIN9 R17',
    'database': baseline.get('nextSqlNumber') == 205 and str(baseline.get('sqlRevision')) == '199-204',
    ...
    'cache': 'nine-cloud-dao-v1.8.6-cache88-bsect04-socialspar1' in sw and build_id in sw,
    'no-secrets': 'sb_secret_' not in '\n'.join(text(x) for x in ['app.js','config.js','index.html']).lower(),
    'sect-core': all((root / x).is_file() for x in ['b-sect-v2.js','b-sect-v2.css']) and all(t in sect for t in ['function tabNav()','function metric(label, value','sync_sect_v2_dashboard_bsect04']),
    ...
    'sect-sparring': all(t in sect for t in ['set_sect_sparring_lineup_bsect04','start_sect_sparring_bsect04','必须恰好选择三名弟子']),
