...
build='v2-2-0-cache135-b-ai-router-glm-admin39-sql263-gated'; label='V2.2.0 CACHE135'
required=[
...
'config':all(x in text('config.js') for x in [label,build,'cacheEpoch: 135']),
'release-json':release.get('releaseLabel')==label and release.get('cacheEpoch')==135 and release.get('buildId')==build and release.get('nextSqlNumber')==264 and release.get('gmVersion')=='ADMIN9 R39' and release.get('runtimeDatabaseGate')=='SQL263_GATE_PASSED' and release.get('edgeDeploymentRequired') is True,
'baseline-json':baseline.get('releaseLabel')==label and baseline.get('nextSqlNumber')==264 and 'SQL263' in baseline.get('sqlRevision','') and baseline.get('gmVersion')=='ADMIN9 R39' and baseline.get('androidVersionCode')==2001514,
'index-cache-bust':build in idx and '<!-- version: V2.2.0 CACHE135 -->' in idx and 'b-exploration-v220.js' in idx,
...
'sw-cache135':build in sw and 'b-exploration-v220.js' in sw,
...
(out/'PAGES_ARTIFACT_MANIFEST.json').write_text(json.dumps({'version':label,'clientBuild':build,'requiredDatabaseGate':'SQL263_GATE_PASSED','files':files},ensure_ascii=False,indent=2)+'\n','utf-8')
