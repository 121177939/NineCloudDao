...
build='v2-1-1-cache114-equipped-detail-admin25-sql244'
label='V2.1.1 CACHE114'
required=[
...
checks={
 ...
 'config':all(x in text('config.js') for x in [label,build,'cacheEpoch: 114']),
 'release-json':release.get('releaseLabel')==label and release.get('cacheEpoch')==114 and release.get('buildId')==build and release.get('nextSqlNumber')==245,
 'database':baseline.get('nextSqlNumber')==245 and baseline.get('sqlRevision')=='211-221 + 229-244',
 ...
 'cache114-release':release.get('gmVersion')=='ADMIN9 R25' and release.get('migrationSql')==[] and release.get('runtimeDatabaseGate')=='SQL244_PASSED_NO_NEW_SQL_FOR_CACHE114',
 'cache114-equipped-detail':"if (inBag) actions.push('<button class=\"primary-btn\" data-eq-action=\"forge\">孔位 / 升品 / 破境</button>');" in text('b-equipment01.js') and "inBag || equipped" not in text('b-equipment01.js'),
