...
run('release metadata',[py,'tools/verify_release_v1_2_fix3.py','.'])
for rel in ['app.js','config.js','update-guard.js','sw.js','b-paigow01.js','paigow-app.js']:
 run('node syntax '+rel,['node','--check',rel])
for rel in ['tools/prepare_pages_site_v1_2_fix3.py','tools/verify_pages_site_v1_2_fix3.py','tools/verify_release_v1_2_fix3.py','tools/sql_static_audit_v1_2_fix3.py','tools/test_paigow_stable_ui_cache40.py','tools/ci_v1_2_fix3.py']:
 run('python syntax '+rel,[py,'-m','py_compile',rel])
run('V1.2 FIX3 SQL audit',[py,'tools/sql_static_audit_v1_2_fix3.py','.'])
run('V1.2 FIX3 feature test',['node','tools/test_features_v1_2_fix3.js','.'])
run('stable UI browser test',[py,'tools/test_paigow_stable_ui_cache40.py','.'])
...
run('build pages',[py,'tools/prepare_pages_site_v1_2_fix3.py','.','.pages-site'])
run('verify pages',[py,'tools/verify_pages_site_v1_2_fix3.py','.pages-site'])
print('\nV1.2 FIX3 CACHE40 CI PASS')
