...
build='v2-2-0-cache122-combat-techniques-admin33-sql254-r5'
label='V2.2.0 CACHE122'
required=[
...
checks={
 ...
 'config':all(x in text('config.js') for x in [label,build,'cacheEpoch: 122']),
 'release-json':release.get('releaseLabel')==label and release.get('cacheEpoch')==122 and release.get('buildId')==build and release.get('nextSqlNumber')==255 and release.get('gmVersion')=='ADMIN9 R33',
 ...
 'new-assets-index':all(x in text('index.html') for x in ['b-technique-v220.js','b-technique-v220.css','<!-- version: V2.2.0 CACHE122 -->']),
 ...
 'pages-workflow-v220':all(x in text('.github/workflows/deploy-pages.yml') for x in [label,'build_pages_v2_2_0_cache122.py','b-technique-v220.js']),
