...
build='v2-1-1-cache110-forgeui2-admin22-sql239'
label='V2.1.1 CACHE110'
required=[
...
checks={
 ...
 'config':all(x in text('config.js') for x in [label,build,'cacheEpoch: 110']),
 'release-json':release.get('releaseLabel')==label and release.get('cacheEpoch')==110 and release.get('buildId')==build and release.get('nextSqlNumber')==240,
 'database':baseline.get('nextSqlNumber')==240 and baseline.get('sqlRevision')=='211-221 + 229-239',
