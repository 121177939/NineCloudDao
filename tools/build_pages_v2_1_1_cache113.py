...
build='v2-1-1-cache113-forgecost-combatguard-admin25-sql243'
label='V2.1.1 CACHE113'
required=[
...
checks={
 ...
 'config':all(x in text('config.js') for x in [label,build,'cacheEpoch: 113']),
 'release-json':release.get('releaseLabel')==label and release.get('cacheEpoch')==113 and release.get('buildId')==build and release.get('nextSqlNumber')==244,
 'database':baseline.get('nextSqlNumber')==244 and baseline.get('sqlRevision')=='211-221 + 229-243',
 ...
 'cache113-release':release.get('gmVersion')=='ADMIN9 R25' and release.get('migrationSql')==[242,243] and release.get('runtimeDatabaseGate')=='SQL242_THEN_SQL243_PENDING_USER_EXECUTION',
 'cache113-sql-files':(root/'database-upgrades/SQL242_CACHE112_ADMIN9_R24_DB_HEALTH'/'242_V2.1.1_CACHE112_ADMIN9_R24_数据库健康监控与快照.sql').is_file() and (root/'database-upgrades/SQL243_CACHE113_FORGE_COST_COMBAT_GUARD'/'243_V2.1.1_CACHE113_孔位灵石消耗与战斗接入检查.sql').is_file(),
