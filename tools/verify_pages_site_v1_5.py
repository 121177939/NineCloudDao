...
ck('version',text('VERSION.txt').strip()=='V1.5')
ck('config',all(x in text('config.js') for x in ["buildId: 'v1-5-cache43'",'cacheEpoch: 43']))
ck('index','b-paigow01.js?v=v1-5-cache43' in text('index.html'))
ck('iframe','paigow-app.js?v=v1-5-cache43' in text('b-paigow01.html'))
ck('sw','nine-cloud-dao-v1.5-cache43' in text('sw.js'))
ck('feature-assets','minimum_entry_balance' in text('paigow-app.js') and 'pg-resource-fx-layer' in text('paigow-app.css'))
manifest=json.loads(text('PAGES_ARTIFACT_MANIFEST.json'));ck('manifest-meta',manifest.get('version')=='V1.5 CACHE43' and manifest.get('clientBuild')=='v1-5-cache43')
