...
for(const [name,html,tokens] of [['lobby',lobby,['大堂','贵宾雅间','全服造化池','已取消每日次数限制','统一灵石']],['house',house,['灵骰问道','气运龟卜','赌注资源','快捷倍数','自定义赌注数量','确认落注并立即开局']],['duel',duel,['贵宾雅间','灵拳对弈','五行灵拳','确认赌注并封招开桌']],['pools',pools,['40%','60%','重复游玩','不会叠加个人中奖权重','未中滚存']]]) for(const t of tokens) if(!html.includes(t)) throw new Error(`${name} missing ${t}`);
...
console.log(JSON.stringify({ok:true,checks:42,lobby:lobby.length,house:house.length,duel:duel.length,pools:pools.length}));
