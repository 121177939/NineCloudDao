...
for(const [name,html,tokens] of [['lobby',lobby,['大堂','贵宾雅间','全服造化池','已取消每日次数限制','统一灵石']],['house',house,['灵骰问道','气运龟卜','赌注资源','快捷倍数','自定义赌注数量','确认落注并立即开局','3—10点为小','11—18点为大','111/222/333归小','大小各50%','普通豹子','1/80','1/5000','毛利润3倍','毛利润34倍','败局下注额10%','余下90%']],['duel',duel,['贵宾雅间','灵拳对弈','五行灵拳','确认赌注并封招开桌','双方各押100','胜者共到账195','奖池增加5']],['pools',pools,['40%','60%','大堂赢局按毛利润5%进入奖池','败者赌注的5%进入奖池、95%转给胜者','不会叠加个人中奖权重','未中滚存']]]) for(const t of tokens) if(!html.includes(t)) throw new Error(`${name} missing ${t}`);
...
if(house.includes('押围骰')||house.includes('value="triple"')) throw new Error('independent triple bet should be removed');
f.state.casinoDrafts.house={stakeType:'spirit_stone',amount:100,multiplier:null,game:'spirit_dice',choice:'triple'};
if(f.getCasinoDraft('house',data).choice!=='big') throw new Error('legacy triple draft was not sanitized');
...
console.log(JSON.stringify({ok:true,checks:50,lobby:lobby.length,house:house.length,duel:duel.length,pools:pools.length}));
