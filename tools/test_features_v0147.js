...
const checks=[
 ...
 ['player stake superseded safely',app.includes('当前可用资源的10%')&&fs.readFileSync(path.join(root,'SQL/45_V1.0_FIX4_赌坊安全结算.sql'),'utf8').includes("'player_house_unlimited_stake',false")],
 ['system cover superseded safely',app.includes('不再兜底')&&fs.readFileSync(path.join(root,'SQL/45_V1.0_FIX4_赌坊安全结算.sql'),'utf8').includes('system_cover_amount=0')],
