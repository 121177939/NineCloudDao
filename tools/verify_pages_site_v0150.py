...
ck('cache','v0150-cache16' in txt('config.js'))
ck('epoch','cacheEpoch: 16' in txt('config.js'))
ck('continuous queue','enqueueFishShrimpBetV0150' in txt('app.js') and 'processFishShrimpBetQueueV0150' in txt('app.js'))
ck('no busy lock','落注中…' not in txt('app.js'))
...
m=json.loads(txt('PAGES_ARTIFACT_MANIFEST.json'));ck('manifest',m.get('clientBuild')=='v0150-cache16')
