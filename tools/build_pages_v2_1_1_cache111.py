...
build='v2-1-1-cache111-totalstatsfix-admin22-sql240'
label='V2.1.1 CACHE111'
required=[
...
checks={
 ...
 'config':all(x in text('config.js') for x in [label,build,'cacheEpoch: 111']),
 'release-json':release.get('releaseLabel')==label and release.get('cacheEpoch')==111 and release.get('buildId')==build and release.get('nextSqlNumber')==241,
 'database':baseline.get('nextSqlNumber')==241 and baseline.get('sqlRevision')=='211-221 + 229-240',
