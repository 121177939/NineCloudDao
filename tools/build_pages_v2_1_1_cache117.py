...
build='v2-1-1-cache117-equipmentstate-upgradefix-admin27-sql247'
label='V2.1.1 CACHE117'
required=[
...
checks={
 ...
 'config':all(x in text('config.js') for x in [label,build,'cacheEpoch: 117']),
 'release-json':release.get('releaseLabel')==label and release.get('cacheEpoch')==117 and release.get('buildId')==build and release.get('nextSqlNumber')==248,
 'database':baseline.get('nextSqlNumber')==248 and 'SQL247 REQUIRED' in baseline.get('sqlRevision',''),
 ...
 'cache117-release':release.get('gmVersion')=='ADMIN9 R27' and release.get('migrationSql')==[245,246,247] and 'SQL247_GATE_PASSED' in release.get('runtimeDatabaseGate',''),
 'cache117-equipped-detail':"if (inBag) actions.push('<button class=\"primary-btn\" data-eq-action=\"forge\">孔位 / 升品 / 破境</button>');" in text('b-equipment01.js') and "inBag || equipped" not in text('b-equipment01.js'),
