...
const tests = {
  'quiet cave semantic label': app.includes('aria-label="幽静洞府主景与建筑"'),
  ...
  'real storage preserved': app.includes('CAVE_STORAGE_SLOT_COUNT_B01 = 30') && app.includes('caveStorageItemsB01(inventory)'),
  ...
  'cache27 build': fs.readFileSync(path.join(root, 'config.js'), 'utf8').includes("buildId: 'v0155-fix1-cache27'")
};
let failed = 0;
