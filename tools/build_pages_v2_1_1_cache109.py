...
build='v2-1-1-cache109-equipmentdetail1-admin22-sql238'
label='V2.1.1 CACHE109'
required=[
...
checks={
 ...
 'config':all(x in text('config.js') for x in [label,build,'cacheEpoch: 109']),
 'release-json':release.get('releaseLabel')==label and release.get('cacheEpoch')==109 and release.get('buildId')==build and release.get('nextSqlNumber')==239,
 'database':baseline.get('nextSqlNumber')==239 and baseline.get('sqlRevision')=='211-221 + 229-238',
