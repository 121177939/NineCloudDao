...
build='v2-2-0-cache131-complete-tiandao-people-admin38-sql260-ready'; label='V2.2.0 CACHE131'
required=[
...
'config':all(x in text('config.js') for x in [label,build,'cacheEpoch: 131']),
'release-json':release.get('releaseLabel')==label and release.get('cacheEpoch')==131 and release.get('buildId')==build and release.get('nextSqlNumber')==261 and release.get('gmVersion')=='ADMIN9 R38' and release.get('runtimeDatabaseGate')=='SQL260_GATE_PASSED' and release.get('edgeDeploymentRequired') is False,
'baseline-json':baseline.get('releaseLabel')==label and baseline.get('nextSqlNumber')==261 and 'SQL260 READY' in baseline.get('sqlRevision','') and baseline.get('gmVersion')=='ADMIN9 R38' and baseline.get('androidVersionCode')==2001510,
'index-cache-bust':build in idx and '<!-- version: V2.2.0 CACHE131 -->' in idx,
...
'sw-cache131':build in sw and 'cache130' not in sw,
}
