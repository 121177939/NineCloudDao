...
build='v2-1-1-cache115-newserver-admin26r2-sql245'
label='V2.1.1 CACHE115'
required=[
...
checks={
 ...
 'config':all(x in text('config.js') for x in [label,build,'cacheEpoch: 115']),
 'release-json':release.get('releaseLabel')==label and release.get('cacheEpoch')==115 and release.get('buildId')==build and release.get('nextSqlNumber')==245,
 'database':baseline.get('nextSqlNumber')==245 and 'SQL245 R2待执行' in baseline.get('sqlRevision',''),
 ...
 'cache115-release':release.get('gmVersion')=='ADMIN9 R26 R2' and release.get('migrationSql')==[245] and release.get('runtimeDatabaseGate')=='SQL245_GATE_PASSED_REQUIRED_FOR_CACHE115',
 'cache115-equipped-detail':"if (inBag) actions.push('<button class=\"primary-btn\" data-eq-action=\"forge\">孔位 / 升品 / 破境</button>');" in text('b-equipment01.js') and "inBag || equipped" not in text('b-equipment01.js'),
