...
ck('version',txt('VERSION.txt').strip()=='V0.14.3')
ck('html-version',all('0.14.3' in txt(f) for f in ['index.html','404.html']))
ck('config-version',"version: '0.14.3'" in txt('config.js'))
ck('sw-cache','nine-cloud-dao-v0.14.3-cache7' in txt('sw.js'))
ck('html-cache-build',all(t in txt('index.html') for t in ['styles.css?v=0143-cache7','update-guard.js?v=0143-cache7','app.js?v=0143-cache7']))
ck('guard-rpc','get_jiuxiao_app_release_control_v1' in txt('update-guard.js'))
ck('ranking-tabs',all(t in txt('app.js') for t in ['修为榜','财富榜','战力榜','refreshRankingBoard']))
...
m=json.loads(txt('PAGES_ARTIFACT_MANIFEST.json')); ck('manifest.version',m.get('version')=='V0.14.3' and m.get('clientBuild')=='v0143-cache7')
