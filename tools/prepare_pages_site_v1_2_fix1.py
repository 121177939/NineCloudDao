...
for rel in files:
 ...
 dst=out/rel;dst.parent.mkdir(parents=True,exist_ok=True);shutil.copy2(src,dst);data=dst.read_bytes();manifest.append({'path':rel,'size':len(data),'sha256':hashlib.sha256(data).hexdigest()})
(out/'PAGES_ARTIFACT_MANIFEST.json').write_text(json.dumps({'version':'V1.2 FIX1 CACHE38','clientBuild':'v1-2-fix1-cache38','files':manifest},ensure_ascii=False,indent=2)+'\n','utf-8')
