...
ck('version',txt('VERSION.txt').strip()=='V0.14.4')
ck('html-version',all('0.14.4' in txt(f) for f in ['index.html','404.html']))
ck('config-version',"version: '0.14.4'" in txt('config.js'))
ck('sw-cache','nine-cloud-dao-v0.14.4-cache8' in txt('sw.js'))
ck('html-cache-build',all(t in txt('index.html') for t in ['styles.css?v=0144-cache8','update-guard.js?v=0144-cache8','app.js?v=0144-cache8']))
ck('guard-rpc','get_jiuxiao_app_release_control_v1' in txt('update-guard.js'))
ck('ranking-tabs',all(t in txt('app.js') for t in ['修为榜','财富榜','战力榜','refreshRankingBoard','rpcGetCasinoPlayerHouseStatusV1','showDivineNoticeModal']))
ck('no-ranking-refresh','data-ranking-refresh' not in txt('app.js'))
...
m=json.loads(txt('PAGES_ARTIFACT_MANIFEST.json')); ck('manifest.version',m.get('version')=='V0.14.4' and m.get('clientBuild')=='v0144-cache8')
