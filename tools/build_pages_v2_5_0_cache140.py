...
build='v2-5-0-cache140-equipment-earth-decompose-admin41-sql268r1'; label='V2.5.0 CACHE140'
...
'config':all(x in text('config.js') for x in [label,build,'cacheEpoch: 140']),
'release':release.get('releaseLabel')==label and release.get('cacheEpoch')==140 and release.get('buildId')==build and release.get('nextSqlNumber')==269 and release.get('gmVersion')=='ADMIN9 R41' and release.get('runtimeDatabaseGate')=='SQL267_GATE_PASSED',
'baseline':baseline.get('releaseLabel')==label and baseline.get('buildId')==build and baseline.get('nextSqlNumber')==269 and baseline.get('gmVersion')=='ADMIN9 R41' and baseline.get('androidVersionCode')==2001519,
'index':build in idx and '<!-- version: V2.5.0 CACHE140 -->' in idx and 'b-spirit-beast-v250.js' in idx and 'b-spirit-beast-v250.css' in idx,
'client':all(x in app for x in ['CACHE139','openCaveFeatureModalB01']) and 'id="caveWorkbenchB01"' not in app and all(x in text('b-equipment01.js') for x in ['data-batch-earth','一键选择地品',"item?.grade_code === 'earth'",'decompose_equipment_v180']) and all(x in beast for x in ['get_spirit_beast_hub_v267','spirit_beast_capture_v267','spirit_beast_hatch_egg_v267','spirit_beast_evolve_v267','spirit_beast_inherit_bloodline_v267','get_spirit_beast_ranking_v267']),
