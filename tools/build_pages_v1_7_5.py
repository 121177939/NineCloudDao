...
checks={
 'version':(root/'VERSION.txt').read_text('utf-8').strip()=='V1.7.5',
 'config':all(x in (root/'config.js').read_text('utf-8') for x in ["version: '1.7.5'","releaseLabel: 'V1.7.5 CACHE51'","buildId: 'v1-7-5-cache51'",'cacheEpoch: 51']),
 'index':all(x in (root/'index.html').read_text('utf-8') for x in ['b-paigow01.js?v=v1-7-5-cache51','b-equipment01.js?v=v1-7-5-cache51','b-equipment01.css?v=v1-7-5-cache51']),
 'iframe':all(x in (root/'b-paigow01.html').read_text('utf-8') for x in ['paigow-realtime.js?v=v1-7-5-cache51','paigow-app.js?v=v1-7-5-cache51']),
 'service-worker':all(x in (root/'sw.js').read_text('utf-8') for x in ['nine-cloud-dao-v1.7.5-cache51','paigow-realtime.js?v=v1-7-5-cache51','b-equipment01.js?v=v1-7-5-cache51']),
...
(out/'PAGES_ARTIFACT_MANIFEST.json').write_text(json.dumps({'version':'V1.7.5 CACHE51','clientBuild':'v1-7-5-cache51','files':manifest},ensure_ascii=False,indent=2)+'\n','utf-8')
...
print(f'V1.7.5 CACHE51 pages build PASS files={len(manifest)}')
