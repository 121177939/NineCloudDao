...
required=['index.html','404.html','app.js','styles.css','config.js','update-guard.js','sw.js','manifest.webmanifest','VERSION.txt','CURRENT_BASELINE.json','release_config.json','.github/workflows/deploy-pages.yml','database/V0.15.4/202607290090_v0154_precheck.sql','database/V0.15.4/202607290100_v0154_cache23_full_upgrade.sql','database/V0.15.4/202607290110_v0154_check.sql','database/V0.15.4/202607290120_v0154_rollback.sql','SQL/11_V0.15.4_升级前检查.sql','SQL/12_V0.15.4_CACHE23_正式升级.sql','SQL/13_V0.15.4_升级后检查.sql','SQL/14_V0.15.4_回滚.sql','SQL/15_V0.15.4_FIX3_丹药购买堆叠修复.sql','SQL/16_V0.15.4_FIX4_购买即时入包_CACHE24.sql','SQL/17_V0.15.4_FIX4_最终检查.sql','tools/prepare_pages_site_v0154.py','tools/verify_pages_site_v0154.py','tools/sql_static_audit_v0154.py','tools/test_features_v0154.js','tools/ci_v0154.py']
for r in required:ck(f'required:{r}',(root/r).is_file())
ck('version',txt('VERSION.txt').strip()=='V0.15.4')
for r in ['index.html','404.html','config.js','sw.js','manifest.webmanifest']:ck(f'cache24:{r}','0154-cache24' in txt(r) or '0.15.4-cache24' in txt(r))
ck('config-version',"version: '0.15.4'" in txt('config.js'));ck('config-epoch','cacheEpoch: 24' in txt('config.js'))
release=json.loads(txt('release_config.json'));baseline=json.loads(txt('CURRENT_BASELINE.json'))
ck('release-build',release.get('clientBuild')=='v0154-cache24');ck('release-version',release.get('version')=='V0.15.4');ck('release-migration',release.get('databaseMigration')=='database/V0.15.4/202607290100_v0154_cache23_full_upgrade.sql')
ck('baseline-version',baseline.get('version')=='0.15.4');ck('baseline-hotfix',baseline.get('clientHotfix')=='V0.15.4_FIX4_CACHE24')
