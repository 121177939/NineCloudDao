...
ck('version',txt('VERSION.txt').strip()=='V0.14.8 FIX1'); ck('cache','v0148-fix1-cache13' in txt('config.js')); ck('epoch','cacheEpoch: 13' in txt('config.js')); ck('feature','rpcGetFishShrimpStateV0148' in txt('app.js')); ck('entry-fix','houseGameInput.value = game' in txt('app.js'))
m=json.loads(txt('PAGES_ARTIFACT_MANIFEST.json')); ck('manifest',m.get('clientBuild')=='v0148-fix1-cache13')
