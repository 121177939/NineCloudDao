...
ck('config',all(x in t('config.js') for x in ["version: '1.0-fix3'","buildId: 'v1-fix3-cache33'",'cacheEpoch: 33']))
ck('index-cache','v1-fix3-cache33' in t('index.html'));ck('sw-cache','nine-cloud-dao-v1.0-fix3-cache33' in t('sw.js'))
...
m=json.loads(t('PAGES_ARTIFACT_MANIFEST.json'));ck('manifest-version',m.get('version')=='V1.0 FIX3 CACHE33');ck('manifest-build',m.get('clientBuild')=='v1-fix3-cache33')
