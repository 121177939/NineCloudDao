...
build='v2-5-0-cache138-spirit-beast-complete-admin41-sql267-gated'; label='V2.5.0 CACHE138'
...
gm=root/'九霄问道_ADMIN9_R41_V2.5.0_CACHE138_SQL267_灵兽完整管理_手机直用版.html'
checks={
'version':text('VERSION.txt').splitlines()[0]==label and build in text('VERSION.txt'),
'config':all(x in text('config.js') for x in [label,build,'cacheEpoch: 138']),
'release':release.get('releaseLabel')==label and release.get('cacheEpoch')==138 and release.get('buildId')==build and release.get('nextSqlNumber')==268 and release.get('gmVersion')=='ADMIN9 R41' and release.get('runtimeDatabaseGate')=='SQL267_GATE_PASSED',
'baseline':baseline.get('releaseLabel')==label and baseline.get('buildId')==build and baseline.get('nextSqlNumber')==268 and baseline.get('gmVersion')=='ADMIN9 R41' and baseline.get('androidVersionCode')==2001517,
'index':build in idx and '<!-- version: V2.5.0 CACHE138 -->' in idx and 'b-spirit-beast-v250.js' in idx and 'b-spirit-beast-v250.css' in idx,
'client':all(x in app for x in ['data-cave-open-panel="beasts"','spiritBeastRootV267','jiuxiao:spirit-beast-rendered']) and all(x in beast for x in ['get_spirit_beast_hub_v267','spirit_beast_capture_v267','spirit_beast_hatch_egg_v267','spirit_beast_evolve_v267','spirit_beast_inherit_bloodline_v267','get_spirit_beast_ranking_v267']),
