...
ck('config',all(x in text('config.js') for x in ["buildId: 'v1-6-fix1-cache45'",'cacheEpoch: 45']))
ck('index','b-paigow01.js?v=v1-6-fix1-cache45' in text('index.html'))
ck('iframe','paigow-realtime.js?v=v1-6-fix1-cache45' in text('b-paigow01.html') and 'paigow-app.js?v=v1-6-fix1-cache45' in text('b-paigow01.html'))
ck('sw','nine-cloud-dao-v1.6-fix1-cache45' in text('sw.js') and 'paigow-realtime.js?v=v1-6-fix1-cache45' in text('sw.js'))
ck('no-sql-in-pages',not any(root.rglob('*.sql')))
manifest=json.loads(text('PAGES_ARTIFACT_MANIFEST.json'));ck('manifest-meta',manifest.get('version')=='V1.6 FIX1 CACHE45' and manifest.get('clientBuild')=='v1-6-fix1-cache45')
