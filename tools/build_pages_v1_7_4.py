...
checks={
 'version':(root/'VERSION.txt').read_text('utf-8').strip()=='V1.7.4',
 'config':all(x in (root/'config.js').read_text('utf-8') for x in ["version: '1.7.4'","releaseLabel: 'V1.7.4 CACHE50'","buildId: 'v1-7-4-cache50'",'cacheEpoch: 50']),
 'index':all(x in (root/'index.html').read_text('utf-8') for x in ['b-paigow01.js?v=v1-7-4-cache50','b-equipment01.js?v=v1-7-4-cache50','b-equipment01.css?v=v1-7-4-cache50']),
 'iframe':all(x in (root/'b-paigow01.html').read_text('utf-8') for x in ['paigow-realtime.js?v=v1-7-4-cache50','paigow-app.js?v=v1-7-4-cache50']),
 'service-worker':all(x in (root/'sw.js').read_text('utf-8') for x in ['nine-cloud-dao-v1.7.4-cache50','paigow-realtime.js?v=v1-7-4-cache50','b-equipment01.js?v=v1-7-4-cache50']),
...
(out/'PAGES_ARTIFACT_MANIFEST.json').write_text(json.dumps({'version':'V1.7.4 CACHE50','clientBuild':'v1-7-4-cache50','files':manifest},ensure_ascii=False,indent=2)+'\n','utf-8')
...
print(f'V1.7.4 CACHE50 pages build PASS files={len(manifest)}')
