...
build='v2-2-0-cache128-tiandao-people-admin35-sql259'; label='V2.2.0 CACHE128'
required=[
...
new_rpc=['get_tiandao_people_hub_v1','get_tiandao_person_detail_v1','resolve_tiandao_encounter_v1','tiandao_npc_interact_v1','tiandao_romance_action_v1','tiandao_companion_action_v1']
checks={
...
'config':all(x in text('config.js') for x in [label,build,'cacheEpoch: 128']),
'release-json':release.get('releaseLabel')==label and release.get('cacheEpoch')==128 and release.get('buildId')==build and release.get('nextSqlNumber')==260 and release.get('gmVersion')=='ADMIN9 R35' and release.get('minimumDatabaseSql')==259,
'baseline-json':baseline.get('releaseLabel')==label and baseline.get('nextSqlNumber')==260 and 'SQL259' in baseline.get('sqlRevision','') and baseline.get('gmVersion')=='ADMIN9 R35',
'index-tiandao':all(x in idx for x in ['b-tiandao-person-v220.css','b-tiandao-person-v220.js','<!-- version: V2.2.0 CACHE128 -->']),
...
'tiandao-six-rpc':all(x in bjs for x in new_rpc),
...
'tiandao-no-secret':not re.search(r'CLOUDFLARE_AUTH_TOKEN\s*=|service_role\s*=|sb_secret_',bjs,re.I),
