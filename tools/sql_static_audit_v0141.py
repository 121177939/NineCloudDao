...
p=root/'database/V0.14.1/202607261700_v0141_spirit_stone_casino.sql'; s=p.read_text('utf-8'); p2=root/'database/V0.14.1/202607261930_v0141_fix2_loss_pool_rate.sql'; s2=p2.read_text('utf-8'); p3=root/'database/V0.14.1/202607262030_v0141_fix3_house_win_pool_rate.sql'; s3=p3.read_text('utf-8'); p4=root/'database/V0.14.1/202607262130_v0141_fix4_duel_pool_payout.sql'; s4=p4.read_text('utf-8'); checks=[]
def ck(n,o,d=''): checks.append((n,bool(o),d))
ck('file',p.is_file()); ck('fix2-file',p2.is_file()); ck('fix3-file',p3.is_file()); ck('fix4-file',p4.is_file()); ck('transaction',bool(re.search(r'(?mi)^begin;\s*$',s)) and bool(re.search(r'(?mi)^commit;\s*$',s)))
