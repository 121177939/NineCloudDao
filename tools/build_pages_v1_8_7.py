...
checks = {
    ...
    'config': all(t in text('config.js') for t in [f"releaseLabel: '{label}'", f"buildId: '{build_id}'", 'cacheEpoch: 89']),
    'baseline': baseline.get('sourceBaseline') == 'V1.8.6 CACHE88' and baseline.get('clientHotfix') == 'BSECT05_PARTNERSEASON1' and baseline.get('gmVersion') == 'ADMIN9 R18',
    'database': baseline.get('nextSqlNumber') == 210 and str(baseline.get('sqlRevision')) == '205-209',
    ...
    'cache': 'nine-cloud-dao-v1.8.7-cache89-bsect05-partnerseason1' in sw and build_id in sw,
    'no-secrets': 'sb_secret_' not in '\n'.join(text(x) for x in ['app.js','config.js','index.html']).lower(),
    'sect-core': all((root / x).is_file() for x in ['b-sect-v2.js','b-sect-v2.css']) and all(t in sect for t in ['function tabNav()','function metric(label, value','sync_sect_v2_dashboard_bsect05']),
    'sect-social': all(t in sect for t in ['function peopleHtml','function decisionsHtml','function sparringHtml','set_sect_disciple_position_bsect04','set_sect_master_apprentice_bsect04','resolve_sect_pending_event_bsect04']),
    'sect-sparring': all(t in sect for t in ['set_sect_sparring_lineup_bsect04','start_sect_sparring_bsect04','claim_sect_sparring_daily_reward_bsect05']),
    'sect-partner': all(t in sect for t in ['function partnersHtml','propose_sect_dao_partner_bsect05','resolve_sect_dao_partner_bsect05','set_sect_auto_rules_bsect05']),
    'sect-eventfix': all(t in sect for t in ['sync_sect_v2_phase2_dashboard_bsect03','function eventsHtml','async function autoSettleDue']),
