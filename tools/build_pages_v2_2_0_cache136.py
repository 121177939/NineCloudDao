...
build='v2-2-0-cache136-r2-heaven-control-companion15-admin40-sql264-gated'; label='V2.2.0 CACHE136'
required=[
...
sql264=root/'database-upgrades/SQL264_HEAVEN_CONTROL_TIANDAO_COMPANION_CULTIVATION/01_SQL264_R2_V2.2.0_CACHE136_天道固定_权威修炼链_NPC道侣最终1点5倍_ADMIN9_R40_执行即门禁.sql'
...
'config':all(x in text('config.js') for x in [label,build,'cacheEpoch: 136']),
'release-json':release.get('releaseLabel')==label and release.get('cacheEpoch')==136 and release.get('buildId')==build and release.get('nextSqlNumber')==265 and release.get('gmVersion')=='ADMIN9 R40' and release.get('runtimeDatabaseGate')=='SQL264_GATE_PASSED' and 'SQL264 R2 READY' in release.get('databaseRelease','') and 'R6' in release.get('edgeFunction',''),
'baseline-json':baseline.get('releaseLabel')==label and baseline.get('nextSqlNumber')==265 and 'SQL264' in baseline.get('sqlRevision','') and baseline.get('gmVersion')=='ADMIN9 R40' and baseline.get('androidVersionCode')==2001515,
'index-cache-bust':build in idx and '<!-- version: V2.2.0 CACHE136 -->' in idx and 'b-exploration-v220.js' in idx,
...
'sql264':all(x in sql264_text for x in ['SQL264_GATE_PASSED','heaven_balance_control_v264','tiandao_companion_v264','admin9_update_heaven_balance_control_v264','companion_multiplier', '1.5']),
...
'sw-cache136':build in sw and 'b-exploration-v220.js' in sw,
...
(out/'PAGES_ARTIFACT_MANIFEST.json').write_text(json.dumps({'version':label,'clientBuild':build,'requiredDatabaseGate':'SQL264_GATE_PASSED','files':files},ensure_ascii=False,indent=2)+'\n','utf-8')
