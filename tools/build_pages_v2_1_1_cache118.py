...
build='v2-1-1-cache118-pagesbuildfix-equipmentstate-admin27-sql247'
label='V2.1.1 CACHE118'
required=[
...
checks={
 ...
 'config':all(x in text('config.js') for x in [label,build,'cacheEpoch: 118']),
 'release-json':release.get('releaseLabel')==label and release.get('cacheEpoch')==118 and release.get('buildId')==build and release.get('nextSqlNumber')==248,
 ...
 'cache118-release':release.get('gmVersion')=='ADMIN9 R27' and release.get('migrationSql')==[245,246,247] and release.get('runtimeDatabaseGate')=='SQL247_GATE_PASSED_PRODUCTION_NO_NEW_SQL_FOR_CACHE118',
 'cache118-equipped-detail':"if (inBag) actions.push('<button class=\"primary-btn\" data-eq-action=\"forge\">孔位 / 升品 / 破境</button>');" in text('b-equipment01.js') and "inBag || equipped" not in text('b-equipment01.js'),
 ...
 'pages-workflow-cache118':all(x in text('.github/workflows/deploy-pages.yml') for x in ['V2.1.1 CACHE118','build_pages_v2_1_1_cache118.py']) and 'build_pages_v2_1_1_cache116.py' not in text('.github/workflows/deploy-pages.yml'),
