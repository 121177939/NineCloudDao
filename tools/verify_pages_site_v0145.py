...
ck('version',txt('VERSION.txt').strip()=='V0.14.5'); ck('html-version',all('0.14.5' in txt(f) for f in ['index.html','404.html'])); ck('config-version',"version: '0.14.5'" in txt('config.js')); ck('sw-cache','nine-cloud-dao-v0.14.5-cache9' in txt('sw.js'))
ck('html-cache-build',all(t in txt('index.html') for t in ['styles.css?v=0145-cache9','update-guard.js?v=0145-cache9','app.js?v=0145-cache9']))
ck('v4-ui',all(t in txt('app.js') for t in ['rpcSettleOpportunityV4','showOpportunityOfflineSummary','玩家庄赢家毛利润5%佣金']))
...
m=json.loads(txt('PAGES_ARTIFACT_MANIFEST.json')); ck('manifest.version',m.get('version')=='V0.14.5' and m.get('clientBuild')=='v0145-cache9')
