...
create table if not exists public.world_event_settings (
...
insert into public.world_event_settings(singleton_id)
values (1)
...
create table if not exists public.world_events (
  ...
  constraint world_events_source_unique unique (source_table, source_key, event_type)
);

create index if not exists world_events_public_feed_idx
  on public.world_events(is_public, is_pinned desc, created_at desc);
create index if not exists world_events_actor_idx
  on public.world_events(actor_character_id, created_at desc);
create index if not exists world_events_type_idx
  on public.world_events(event_type, created_at desc);

alter table public.world_event_settings enable row level security;
alter table public.world_events enable row level security;
revoke all on table public.world_event_settings from public, anon, authenticated;
revoke all on table public.world_events from public, anon, authenticated;
...
begin
  ...
  from public.world_event_settings s
...
begin
  begin
    select s.breakthrough_enabled into v_cfg from public.world_event_settings s where s.singleton_id = 1;
...
begin
  begin
    select s.opportunity_enabled into v_cfg from public.world_event_settings s where s.singleton_id = 1;
...
begin
  begin
    select s.casino_enabled into v_cfg from public.world_event_settings s where s.singleton_id = 1;
    if not coalesce(v_cfg, true) then return new; end if;
...
begin
  ...
  select s.admin_enabled into v_cfg from public.world_event_settings s where s.singleton_id = 1;
  ...
  v_event_id := public.world_event_publish_v0140(
    v_world_id, v_world_year, 'admin_account_erasure', 4,
    null, p_character_name, '天道裁决',
    format('修士【%s】%s，已被天道抹去名籍，自此不存于九霄界中。', p_character_name, v_reason),
    'admin_account_erasure', v_key,
...
select public.world_event_publish_v0140(
  ...
  'system_world_feed_open', 2, null, '天道', '界闻初启',
  '九霄天机重新汇聚，自今日起，重大破境、珍稀机缘与赌坊胜负皆将传遍诸域。',
  'system', 'v0140-world-feed-open', jsonb_build_object('version', '0.14.0'), true, null
);

comment on table public.world_events is 'V0.14.0：九霄界闻全服重大事件，只能通过受保护函数和触发器写入。';
comment on function public.get_world_events_v1(integer) is 'V0.14.0：认证玩家读取最近九霄界闻。';
comment on function public.admin_publish_account_erasure_v1(text,text,text,text) is 'V0.14.0：仅service_role/数据库管理员可调用的天道抹杀公开播报接口。';
