-- 《九霄问道》Web Alpha V0.14.0 部署检查。
with checks(name, ok, detail) as (
  values
    ('world_event_settings_table', to_regclass('public.world_event_settings') is not null, '配置表存在'),
    ('world_events_table', to_regclass('public.world_events') is not null, '界闻表存在'),
    ...
    ('rls_world_events', coalesce((select relrowsecurity from pg_class where oid='public.world_events'::regclass), false), 'world_events启用RLS'),
    ('unique_source', exists(select 1 from pg_constraint where conname='world_events_source_unique'), '来源幂等约束存在'),
    ('opening_notice', exists(select 1 from public.world_events where source_table='system' and source_key='v0140-world-feed-open'), '启用公告存在')
)
...
begin
  if exists (
    with checks(ok) as (
      values
        (to_regclass('public.world_event_settings') is not null),
