-- V0.14.0 安全回滚：移除触发器与公开RPC，保留world_events历史数据供审计。
begin;
update public.world_event_settings set enabled=false,breakthrough_enabled=false,opportunity_enabled=false,casino_enabled=false,admin_enabled=false,updated_at=now() where singleton_id=1;
