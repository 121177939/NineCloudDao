-- 九霄问道 V0.14.5 正式数据库升级
-- 基线：V0.14.4 AB2 / CACHE8
-- 内容：机缘V4、原12主修+新增12辅修、离线净汇总、玩家庄赢家毛利润5%佣金。
-- A线加固：新增V4表全部启用RLS并撤销客户端直连权限。
-- 执行前必须备份数据库；本文件不代表生产环境已经执行。
...
-- 发布控制：要求CACHE8及更早客户端刷新为V0.14.5 CACHE9。
do $$
begin
  if to_regclass('public.app_release_control') is not null then
    update public.app_release_control
    set release_name='V0.14.5 CACHE9',cache_epoch=greatest(cache_epoch,9),updated_at=now()
    where singleton_id=1;
  end if;
end;
$$;

comment on function public.settle_opportunity_v4(boolean) is 'V0.14.5：在线、离线统一每5分钟结算机缘，离线最多72小时/864次。';
comment on function public.casino_play_player_house_v1(uuid,text,text,bigint,text) is 'V0.14.5：玩家庄胜局从赢家毛利润中保留5%佣金，本金原返，佣金不入造化池。';
...
select case when bool_and(c.relrowsecurity) then 'PASS' else 'FAIL' end result,'v4_tables_rls' check_name,count(*) actual
from pg_class c join pg_namespace n on n.oid=c.relnamespace
where n.nspname='public' and c.relname in('opportunity_v4_story_pool','opportunity_v4_result_pool','opportunity_v4_settlement_batches','opportunity_v4_technique_drop_rates','opportunity_v4_technique_pool');
select case when to_regprocedure('public.heavenly_insight_cultivation_multiplier_v0144(uuid)') is not null then 'PASS' else 'FAIL' end result,'v0144_insight_preserved' check_name;
select case when to_regprocedure('public.claim_next_divine_notice_v1()') is not null then 'PASS' else 'FAIL' end result,'v0144_private_notice_preserved' check_name;
select case when spirit_dice_ordinary_triple_denominator=80 and spirit_dice_destiny_triple_result_denominator=5000 and spirit_dice_ordinary_triple_net_odds=3 and spirit_dice_destiny_triple_net_odds=34 then 'PASS' else 'FAIL' end result,'fix7a_odds_preserved' check_name from public.casino_settings where singleton_id=1;
