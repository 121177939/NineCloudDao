-- 《九霄问道》Web Alpha V0.11.7
-- 天道动态均衡机制：低于全服主流境界获得灵气福泽，贴近主流保持均衡，高于主流受到灵气阻滞。
-- 设计原则：
-- 1. 全服主流境界 = 同一世界在世修士大境界序号的算术平均值四舍五入；
-- 2. 境界差 = 玩家大境界序号 - 全服主流大境界序号；
-- 3. 天道系数只调整“灵气环境”这一项，不将功法、机缘等固定收益整体重复乘算；
-- 4. 不新增业务表，只新增2个函数并替换 claim_cultivation_v1()。
...
-- 替换自动修炼结算：
-- 旧式：全部收益 × 灵气环境倍率。
-- V0.11.7：固定修炼部分按原有灵根/功法/命格计算，再单独加上“灵气环境基础值 × 天道系数”。
...
declare
  ...
  v_qi_base numeric := 1;
  v_qi_environment_gain numeric := 1;
...
begin
  ...
  v_qi_environment_gain := greatest(0, v_qi_base * v_heaven_coefficient);
  ...
  loop
    ...
    -- 天道系数仅作用于灵气环境收益，不重复放大功法、机缘和固定吐纳。
    v_segment_rate := greatest(0, v_segment_fixed_rate + v_qi_environment_gain);
  ...
  v_current_rate := greatest(0, v_current_fixed_rate + v_qi_environment_gain);
  ...
  if v_gained > 0 and v_elapsed >= 300 then
    ...
    select
      ...
      'success',
      jsonb_build_object(
        'mode', 'automatic_v0117_heaven_balance',
        ...
        'qi_environment_gain', v_qi_environment_gain,
