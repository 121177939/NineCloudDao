-- V0.11.7 结构回滚：恢复V0.11.6之前的灵气环境整体乘算规则，并删除天道动态均衡RPC。
-- 会改变修炼计算方式；执行前请备份数据库。
...
begin
  ...
  into v_qi_multiplier
  from public.game_worlds gw
  where gw.id = v_world_id;
  v_qi_multiplier := coalesce(v_qi_multiplier, 1.000000);
  ...
  -- 将领取区间按临时效果开始/结束时间切段，离线期间的临时加成也能精确结算。
...
drop function if exists public.get_heaven_balance_v1();
drop function if exists public.heaven_balance_multiplier_v1(integer);
...
select
  to_regprocedure('public.get_heaven_balance_v1()') is null as balance_rpc_removed,
  to_regprocedure('public.heaven_balance_multiplier_v1(integer)') is null as multiplier_removed;
