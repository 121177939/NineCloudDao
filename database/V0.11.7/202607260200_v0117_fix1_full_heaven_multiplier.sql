-- 《九霄问道》Web Alpha V0.11.7 FIX1
-- 修复：V0.11.7主版本仅将天道系数加在1点/秒左右的灵气环境基础值上，
-- 导致×0.5时总速度几乎不变。FIX1改为：完整自动修炼速度 × 世界灵气 × 天道系数。
-- 前置条件：已执行202607260030_v0117_heaven_balance.sql。
...
begin
  ...
  if v_gained > 0 and v_elapsed >= 300 then
    ...
    select
      ...
      'success',
      jsonb_build_object(
        'mode', 'automatic_v0117_fix1_full_heaven_multiplier',
