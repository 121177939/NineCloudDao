-- 九霄问道 V0.15.1 CACHE18 节奏修正回滚
-- 仅回滚到CACHE17的25/3/7/3/2节奏；不关闭赌场九霄界闻。
...
begin
  ...
  insert into public.casino_fish_rounds_v0148(
    ...
    p_round_no,v_start,v_start+interval '25 seconds',v_start+interval '28 seconds',
    v_start+interval '35 seconds',v_start+interval '40 seconds',v_result
  )
...
begin
  ...
  return jsonb_build_object(
    ...
    'rules',jsonb_build_object(
      'round_seconds',40,'betting_seconds',25,'lock_seconds',3,
      'reveal_seconds',7,'settlement_seconds',3,'next_seconds',2,
...
set betting_closes_at=starts_at+interval '25 seconds',
    reveal_at=starts_at+interval '28 seconds',
    settles_at=starts_at+interval '35 seconds',
    ends_at=starts_at+interval '40 seconds'
where not is_settled and ends_at>now();

update public.jiuxiao_app_release_control
set release_name='V0.15.1 CACHE17',
    cache_epoch=greatest(cache_epoch,18),
    notice_text='已回滚鱼虾灵局节奏修正，恢复V0.15.1 CACHE17时序；赌场界闻播报保持启用。',
    updated_at=now()
