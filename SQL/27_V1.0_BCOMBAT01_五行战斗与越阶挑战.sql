-- 九霄问道 V1.0 · B-COMBAT01 正式迁移
-- 战斗四属性、角色五行、战力榜、越阶挑战、修为转移与九霄界闻战报。
-- 本脚本不写客户端发布门禁；CACHE30 在下一号SQL中独立提升。
begin;
...
-- 元神页只读取当前登录角色的服务端权威战斗快照。
create or replace function public.get_my_battle_snapshot_v1()
returns jsonb language plpgsql stable security definer set search_path=pg_catalog,public,auth,pg_temp as $$
declare v_user_id uuid:=auth.uid();v_character_id uuid;v_snapshot jsonb;
begin
  ...
  v_snapshot:=public.bcombat01_character_snapshot(v_character_id);
  if v_snapshot is null then raise exception 'COMBAT_STATS_NOT_CONFIGURED';end if;
  return v_snapshot;
end$$;
revoke all on function public.get_my_battle_snapshot_v1() from public,anon,authenticated;
grant execute on function public.get_my_battle_snapshot_v1() to authenticated;
