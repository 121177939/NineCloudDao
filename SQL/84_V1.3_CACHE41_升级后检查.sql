-- 九霄问道 V1.3 CACHE41 升级后检查（只读）
select 'release_cache41' check_name,
       exists(select 1 from public.jiuxiao_app_release_control where singleton_id=1 and cache_epoch>=41 and release_name='V1.3 CACHE41') ok,
       '发布门禁已提升到CACHE41' detail
union all
...
union all
select 'mutation_system_retained',to_regprocedure('public.get_my_birth_result_v12()') is not null and exists(select 1 from public.battle_challenge_settings_bcombat01 where singleton_id=1 and mutation_final_damage_bonus=0.08),'变异灵根与战斗体系保持不变';
