(() => {
  ...
  // V1.7.8 CACHE58 装备属性即时刷新：穿戴/卸下后主动重取服务端权威战斗快照。
  window.JIUXIAO_REFRESH_BATTLE_SNAPSHOT_V1 = async function () {
    if (!state.character) return null;
    state.battleSnapshotV1 = null;
    return refreshMyBattleSnapshotV1(true);
  };

  window.addEventListener('jiuxiao:equipment-loadout-changed', () => {
    window.JIUXIAO_REFRESH_BATTLE_SNAPSHOT_V1().catch(error => {
