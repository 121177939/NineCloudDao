(() => {
  ...
  async function refreshBattleSnapshotAfterLoadout(actionName){
    if(!['equip_item_bequipment01','unequip_item_bequipment01'].includes(actionName))return;
    try{
      if(typeof window.JIUXIAO_REFRESH_BATTLE_SNAPSHOT_V1==='function'){
        await window.JIUXIAO_REFRESH_BATTLE_SNAPSHOT_V1();
      }else{
  ...
  async function action(name,body,success){
    ...
    try{
      ...
      await refresh(true);
      await refreshBattleSnapshotAfterLoadout(name);
      if(success)toast(`${success}${['equip_item_bequipment01','unequip_item_bequipment01'].includes(name)?' 战斗属性已同步。':''}`);
    }catch(e){
  ...
  window.B_EQUIPMENT01={refresh,render:renderAll,openBackpack,version:'1.7.8-equip-refresh-r1'};
})();
